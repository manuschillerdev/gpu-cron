...
import report from '../training/report.json' with { type: 'json' };
...
test('JavaScript features and quantized inference match the MLX export', async () => {
  ...
  for (const fixture of fixtures) {
    ...
    for (let i = 0; i < logits.length; i++)
      maximumError = Math.max(maximumError, Math.abs(logits[i]! - fixture.logits[i]!));
  }
...
test('the shipped model matches its provenance report', async () => {
  ...
  for (const [path, hash] of [
    ['../training/data.py', report.dataSourceSha256],
    ['../training/data/manifest.json', report.manifestSha256],
  ] as const) {
    assert.equal(
      createHash('sha256')
  ...
  assert.equal(
    createHash('sha256')
      .update(await readFile(new URL('./model-fixtures.json', import.meta.url)))
      .digest('hex'),
    report.fixturesSha256,
  );
});
