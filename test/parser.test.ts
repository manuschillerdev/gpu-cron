...
import { cpuLogits } from './reference.ts';
mock.method(CronModel, 'create', async () => ({
  executionProvider: 'webgpu',
  outputLocations: ['gpu-buffer'],
  logits: async (input: Float32Array) => cpuLogits(input),
  dispose() {},
}));
...
for (const [input, cron, first] of cases)
  test(input, async () => {
...
test('alternate weeks preserve phase across a month exclusion', async () => {
  const result = await parse('every other Friday at 4pm except in December', {
    ...context,
    reference: '2026-11-20T12:00:00Z',
    count: 5,
  });
  ...
  assert.deepEqual(result.occurrences, [
    '2026-11-20T16:00:00.000Z',
    '2027-01-01T16:00:00.000Z',
    '2027-01-15T16:00:00.000Z',
    '2027-01-29T16:00:00.000Z',
    '2027-02-12T16:00:00.000Z',
  ]);
...
test('last day is calendar-aware and not exported as approximate cron', async () => {
  const result = await parse('on the last day of every month at noon', {
    ...context,
...
test('DST spring gap is skipped', async () => {
  const result = await parse('daily at 02:30', {
    reference: '2026-03-28T02:00:00Z',
...
test('DST autumn fold returns both instants in chronological order', async () => {
  const result = await parse('daily at 02:30', {
    reference: '2026-10-24T23:00:00Z',
    timeZone: 'Europe/Berlin',
    count: 3,
  });
  assert.deepEqual(result.occurrences, [
...
for (const input of [
  ...
  'every 25 hours',
  'every day at 25:00',
  'every friday at 13pm',
  'daily at 9:60',
  'every day at 9am and 5pm',
  'every day at noon until friday',
  'every day at noon except holidays',
  'every monday at noon except monday',
  'every 15 minutes between 9am and 5pm',
  'on the 32nd of every month at noon',
  'tomorrow at noon',
  'every first monday of the month at noon',
  'every day at noon except january-december',
...
test('batch and individual parsing agree; disposed instances reject calls', async () => {
  ...
  assert.deepEqual(
    await parser.parseMany(inputs, context),
    await Promise.all(inputs.map((text) => parser.parse(text, context))),
  );
  parser.dispose();
