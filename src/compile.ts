...
function number(ts: TokenPrediction[]): number {
  ...
  for (let i = 0; i < ts.length; i++) {
    ...
    if (i && !(total >= 20 && total % 10 === 0 && n! >= 1 && n! <= 9 && !/^\d+$/.test(w)))
...
function namedSet(
  ...
  for (let j = 0; j < selected.length; j++) {
    const { t, i } = selected[j]!,
      first = named(t, months);
    result.push(...first);
...
export function compile(
  ...
  try {
    if (family === 'invalid' || !tokens.length || tokens.some((t) => t.role === 'unknown'))
    ...
    if (family !== 'monthly' && get('monthday').length)
    ...
    if (family === 'minutes' || family === 'hours') {
      if (
        ...
        get('minute').length ||
        get('meridiem').length ||
        get('clock-offset').length ||
        get('clock-direction').length
      )
        unsupported('An interval with an additional clock restriction is not supported.');
      if (get('weekday').length)
        unsupported('An interval with included weekdays is not supported.');
      if (get('recurrence').some((t) => /^\d+$/.test(word(t))))
        unsupported('A numeric recurrence token has no predicted quantity role.');
      const quantity = get('quantity');
      const interval = quantity.length ? number(quantity) : 1;
      const field = family === 'minutes' ? 60 : 24;
      if (interval < 1 || interval > field || field % interval !== 0)
        invalid(
          `The interval must divide ${field} evenly to preserve spacing across cron field boundaries.`,
        );
      ...
      const hourTokens = get('hour'),
        minuteTokens = get('minute'),
        offsets = get('clock-offset'),
        directions = get('clock-direction');
      let hour = number(hourTokens),
        minute = minuteTokens.length ? number(minuteTokens) : 0;
      const meridiem = get('meridiem')
        .map(word)
        .filter((w) => w !== 'm');
      if (meridiem.length > 1) unsupported('More than one meridiem was predicted.');
      if (hour < 0 || hour > 23 || minute < 0 || minute > 59)
        invalid('Clock time is out of range.');
      if (meridiem.length) {
        if (
          !['am', 'a', 'morning', 'pm', 'p', 'afternoon', 'evening', 'night'].includes(meridiem[0]!)
        )
          unsupported('Cannot interpret the predicted meridiem.');
        if (hour < 1 || hour > 12)
          invalid('A clock with AM/PM must have an hour between 1 and 12.');
        const pm = ['pm', 'p', 'afternoon', 'evening', 'night'].includes(meridiem[0]!);
        hour = (hour % 12) + (pm ? 12 : 0);
      }
      if (offsets.length) {
        if (minuteTokens.length || directions.length !== 1)
      ...
      if (
        ...
        !minuteTokens.length &&
        !offsets.length &&
        !hourTokens.some((t) => ['noon', 'midnight'].includes(word(t)))
      )
        diagnostics.push({
          code: 'assumption',
          severity: 'info',
      ...
      if (family === 'monthly') {
        ...
        const days = get('monthday');
        if (!days.length) unsupported('The model did not identify a day of the month.');
        ...
        for (const t of days) {
          ...
          if (
            last &&
            /^\s+$/.test(text.slice(last.end, t.start)) &&
            !['last', 'end'].includes(word(t))
          )
            groups.at(-1)!.push(t);
          else groups.push([t]);
        }
        schedule.daysOfMonth = unique(groups.map(number));
        if (schedule.daysOfMonth.some((d) => d !== -1 && (d < 1 || d > 31)))
    ...
    if (excludedDays.length) {
      schedule.weekdays = (schedule.weekdays ?? ALL_DAYS).filter((d) => !excludedDays.includes(d));
      if (!schedule.weekdays.length) invalid('The exclusions remove every scheduled weekday.');
    }
    if (get('range').some((t) => !consumedRanges.has(t)))
