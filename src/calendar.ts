...
export function context(options: ParseOptions): {
  ...
  return {
    ...
    timeZone,
    count,
    anchorWeek: new Date(monday).toISOString().slice(0, 10),
  };
}
...
export function preview(schedule: Schedule, reference: number, count: number): string[] {
  ...
  for (let delta = 0; delta < 366 * 5; delta++) {
    ...
    if (schedule.daysOfMonth) {
      ...
      if (!schedule.daysOfMonth.some((value) => value === day || (value === -1 && day === last)))
        continue;
    }
    const monday = localDay - ((weekday + 6) % 7) * DAY;
    if (schedule.weekInterval === 2 && Math.round((monday - anchor) / (7 * DAY)) % 2 !== 0)
      continue;
