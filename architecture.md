...
The browser performs no semantic interpretation before inference. The compiler reads only the predicted roles, decodes their values and applies calendar arithmetic. For example, `quarter:clock-offset`, `to:clock-direction`, `seven:hour`, `evening:meridiem` becomes 18:45. It never searches the whole sentence to recover a missing role or override the network's family. Unknown roles, unconsumed ranges and inconsistent fields produce diagnostics. Wrongly predicted filler can still discard meaningful language: this remains a measurable model error, not a guaranteed validation boundary.
