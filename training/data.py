...
def token_id(text):
    word = text.lower()
    kind = (
        1 if re.fullmatch("[0-9]+", word) else 0 if re.fullmatch("[a-z]+", word) else 2
    )
...
def number_word(n):
    if n <= 20:
        return NUMBERS[n]
    return {20: "twenty", 30: "thirty", 40: "forty", 50: "fifty"}[n // 10 * 10] + (
        " " + NUMBERS[n % 10] if n % 10 else ""
    )
...
def semantic_labels(item):
    ...
    numbers = [
        ...
        if words[i].isdigit()
        or words[i] in NUMBER_WORDS
        or words[i] in ("noon", "midnight")
    ]
    colon = next(
        (
            i
    ...
    for i, t in enumerate(labels):
        ...
        if c == "unknown":
            ...
            if w in (
                ...
                role = "recurrence"
            elif any(w in (d, d[:3], d + "s") for d in DAYS) or w in (
                "weekday",
                "weekdays",
                "weekend",
                "weekends",
            ...
            role = "prefix"
            if w in ("am", "pm", "morning", "afternoon", "evening", "night") or (
                w in ("a", "p", "m") and "." in words[max(0, i - 1) : i + 3]
            ):
                role = "meridiem"
            elif w in ("quarter", "half"):
                role = "clock-offset"
            elif i == direction:
            ...
            if day:
                role = "excluded-weekday" if excluded else "weekday"
            elif month:
                role = "excluded-month" if excluded else "month"
            elif w in ("through", "to", "-", "–", "—"):
                role = (
                    "range"
                    if not (
                        c == "recurrence" and item["family"] in ("minutes", "hours")
                    )
                    else "recurrence"
                )
            elif (
                not excluded
                and item["family"] == "monthly"
                and (w.isdigit() or w in NUMBER_WORDS or w in ("last", "end"))
            ):
                role = "monthday"
            elif (
                not excluded
                and item["family"] in ("minutes", "hours")
                and (
                    w.isdigit()
                    or w in NUMBER_WORDS
                    or w in ("half", "quarter", "minus")
                )
            ):
                role = "quantity"
            elif w in (",", "/", "&", "+", "(", ")", ".", "!", "?") or w in (
...
def group_id(target):
    ...
    semantic = (
        {k: v for k, v in target["schedule"].items() if k != "family"}
...
def record(parts, family, target, pattern, category, source="generator", identity=None):
    ...
    for part, role in parts:
        ...
        cursor += len(part)
    if [(t.start(), t.end()) for t in tokens(text)] != [
        (t["start"], t["end"]) for t in labels
    ]:
...
def row(item):
    return (
        ...
        [ROLES.index(t["role"]) for t in item["tokens"]],
    )
...
def meaning(rng, index):
    ...
    if family == "invalid":
        ...
        return {
            "family": family,
            "kind": kind,
            "target": {"status": "unsupported", "schedule": None, "reason": kind},
        }
    hour = rng.choice([0, 12]) if rng.random() < 0.2 else rng.randrange(24)
    minute = rng.choice([0, 0, 15, 30, 45]) if rng.random() < 0.7 else rng.randrange(60)
    interval = (
        rng.choice([1, 2, 3, 4, 5, 6, 10, 12, 15, 20, 30])
        if family == "minutes"
        else rng.choice([1, 2, 3, 4, 6, 8, 12])
    )
    weekdays = (
        sorted(rng.sample(range(7), rng.choice([1, 1, 2, 3, 5])))
        if family in ("weekly", "biweekly")
        else None
    )
    if family == "weekly" and rng.random() < 0.35:
        weekdays = rng.choice([[1, 2, 3, 4, 5], [0, 6], [0, 5, 6]])
    monthdays = (
        rng.choice([[-1], [1], [1, 15], [rng.randint(1, 28)]])
        if family == "monthly"
        else None
    )
    excluded_months = (
        sorted(rng.sample(range(1, 13), rng.choice([1, 1, 2, 3, 4])))
        if rng.random() < 0.35
        else []
    )
    excluded_days = (
        rng.choice([[0, 6], [rng.randrange(7)]])
        if not excluded_months and rng.random() < 0.15
        else []
    )
    if weekdays is not None and not set(weekdays) - set(excluded_days):
        excluded_days = []
    target = {
        "status": "supported",
        "schedule": schedule(
            ...
            weekdays,
            monthdays,
            [m for m in range(1, 13) if m not in excluded_months],
            interval,
            excluded_days,
        ),
    }
...
def ordinal(n):
    return str(n) + (
        "th" if 10 < n % 100 < 14 else {1: "st", 2: "nd", 3: "rd"}.get(n % 10, "th")
    )
...
def spoken_clock(hour, minute, rng):
    def hour_text(h):
        ...
        if h % 24 == 12:
            return "noon"
        return number_word(h % 12 or 12) + (
            " in the morning"
            if h % 24 < 12
            else rng.choice([" in the afternoon", " in the evening", " at night"])
        )
    ...
    amount = (
        rng.choice(["a quarter", "quarter", number_word(offset)])
        if offset == 15
        else rng.choice(["half", "thirty"])
        if offset == 30
        else number_word(offset)
    )
    return (
        amount
        + (
            " "
            if "quarter" in amount or amount == "half"
            else rng.choice([" minutes ", " "])
        )
        + rng.choice(["to", "before"] if before else ["past", "after"])
        + " "
        + hour_text(base)
    )
...
def render(m, rng, pattern):
    ...
    if f in ("minutes", "hours"):
        ...
        rec = (
            "hourly"
            if f == "hours" and n == 1 and rng.random() < 0.5
            else f"every {word + ' ' if n != 1 or rng.random() < 0.5 else ''}{unit}{'s' if n != 1 else ''}"
        )
    elif f == "daily":
        rec = rng.choice(
            [
        ...
        if days == [1, 2, 3, 4, 5]:
            day_text = rng.choice(
                ["weekday", "weekdays", "monday through friday", "monday to friday"]
            )
        if days == [0, 6]:
            day_text = rng.choice(["weekend", "weekends", "saturday and sunday"])
        if days == [0, 5, 6]:
            day_text = rng.choice(
                ["friday through sunday", "fri-sun", "fri–sun", "friday to sunday"]
            )
        if (
            len(days) > 1
            and days == list(range(days[0], days[-1] + 1))
            and rng.random() < 0.5
        ):
            day_text = (
                DAYS[days[0]]
                + rng.choice([" through ", " to ", "-", "–"])
                + DAYS[days[-1]]
            )
        rec = (
            rng.choice(["every ", "each ", "on ", "", "every week on ", "weekly on "])
            + day_text
            if f == "weekly"
            else rng.choice(
                [
        ...
        day_text = (
            ...
            + rng.choice([" and ", " + ", ", "]).join(ordinal(d) for d in days)
        )
        if days == [1] and rng.random() < 0.4:
            day_text = "the first"
        if days != [-1] and rng.random() < 0.5:
            day_text = "the " + " and ".join(
                ORDINALS[d] if d <= 20 else ordinal(d) for d in days
            )
        rec = rng.choice(
            [
                f"on {day_text} of every month",
                f"every month on {day_text}",
                f"{day_text} of each month",
                f"monthly on {day_text}",
                f"on {day_text} of the month",
                "on days " + " and ".join(str(d) for d in days) + " each month"
                if days != [-1]
                else "at month end",
                "every month on day " + number_word(days[0])
                if len(days) == 1 and days[0] > 0
                else f"monthly on {day_text}",
            ]
        )
        if days == [-1] and rng.random() < 0.3:
            rec = rng.choice(["at month end", "on each month's last day"])
    if f in ("minutes", "hours"):
        rec = rng.choice(
            [
                rec,
                f"once every {word} {unit}s",
                f"at {word}-{unit} intervals",
                f"every {word} {unit}s",
            ]
        )
        if n == 1:
            rec = rng.choice(
                [rec, f"once a {unit}", "hourly" if f == "hours" else "every minute"]
            )
        if f == "minutes" and n == 30 and rng.random() < 0.5:
            rec = "every half hour"
    ...
    if f not in ("minutes", "hours"):
        ...
        h12 = h % 12 or 12
        clock = rng.choice(
            [f"{h:02d}:{minute:02d}", f"{h12}:{minute:02d}{'am' if h < 12 else 'pm'}"]
        )
        if minute == 0:
            clock = rng.choice(
                [
                    clock,
                    f"{h12}{'am' if h < 12 else 'pm'}",
                    "noon" if h == 12 else "midnight" if h == 0 else str(h),
                ]
            )
        if rng.random() < 0.55:
            clock = spoken_clock(h, minute, rng)
        if rng.random() < 0.08:
            clock = clock.replace(":", ".").replace("am", "a.m.").replace("pm", "p.m.")
        if pattern == "bare-clock":
            clock = str(h)
        time = (rng.choice(["at ", "at ", "", "@ "]) + clock, "time")
        parts = (
            [time, (" " + rec, "recurrence")]
            if pattern == "clock-first"
            else [(rec + " ", "recurrence"), time]
        )
    excluded = m["excluded_months"]
    days = m["excluded_days"]
    if excluded or days:
        names = (
            [MONTHS[v - 1] for v in excluded]
            if excluded
            else ["weekends"]
            if days == [0, 6]
            else [DAYS[v] for v in days]
        )
        if (
            excluded
            and len(excluded) > 1
            and excluded == list(range(excluded[0], excluded[-1] + 1))
            and rng.random() < 0.7
        ):
            names = [
                MONTHS[excluded[0] - 1]
        ...
        intro = (
            ...
            if pattern == "but-not"
                ...
                [
                    ...
                    "unless it's ",
                    "unless it is ",
                    "unless that day is a ",
                    "but never on ",
                ]
            )
            + ("in " if excluded and rng.random() < 0.4 else "")
        )
        exclusion = (intro + " and ".join(names), "exclusion")
        if rng.random() < 0.15:
            parts = [exclusion, (", ", "prefix")] + parts
        elif rng.random() < 0.15:
            parts += [(" (", "prefix"), exclusion, (")", "prefix")]
        else:
            parts.append((" " + exclusion[0], exclusion[1]))
    prefix = rng.choice(
        [
            "",
            "",
            ...
            "trigger this ",
            "I'd like an ",
            "arrange for this to run ",
        ]
    )
    if rng.random() < 0.45:
        prefix = ""
    if prefix and rng.random() < 0.25:
        prefix = (
            rng.choice(
                [
                    "back up",
    ...
    if pattern == "bare-clock":
        prefix = ""
    if prefix:
        parts.insert(0, (prefix, "prefix"))
    if pattern != "bare-clock" and rng.random() < 0.25:
        parts.append(
            (
                rng.choice([".", "!", ", please.", " is when I want this to run."]),
                "prefix",
            )
        )
    if rng.random() < 0.08:
        parts = [(t.upper(), r) for t, r in parts]
    return record(
        parts, f, m["target"], pattern, "exclusions" if excluded or days else f
    )
...
def datasets(epoch=0):
    ...
    for i in range(16000):
        m = meaning(rng, i)
        group = group_id(m["target"])
        if group in reserved or group in seen:
            continue
        seen.add(group)
        bucket = int(group[:8], 16) % 10
        split = (
            "train"
            if m["family"] == "invalid" or bucket < 8
            else "development"
            if bucket == 8
            else "patternHoldout"
        )
        patterns = (
            ["clock-last", "clock-first", "request-prefix"]
            if split != "patternHoldout"
            else ["but-not"]
        )
        if (
            split != "patternHoldout"
            and m["family"] not in ("invalid", "minutes", "hours")
            and m["minute"] == 0
        ):
        ...
        for pattern in patterns * (40 if m["family"] == "invalid" else 1):
            item = render(
                ...
                surface
                if split == "train"
                else random.Random(SEED + i * 31 + len(pattern)),
                pattern,
            )
            item["split"] = split
...
def write_manifest(sets, directory=DATA):
    ...
    for split, items in sets.items():
        (
            directory
            / (("authored" if split == "authoredHoldout" else split) + ".jsonl")
        ).write_text(
            "".join(
        ...
        counts[split] = {
            "examples": len(items),
            "meaningGroups": len({r["groupId"] for r in items}),
    ...
    manifest = {
        ...
        "seed": SEED,
        "splits": counts,
        "grouping": "Canonical structured meaning before rendering; authored meanings reserved from generated splits.",
        "heldoutPattern": "Former but-not holdout is now a meaning-disjoint development stress set; the construction is taught in training.",
        "authoredProvenance": "Assistant-authored and annotated separately from the renderer, not real-user traffic. Texts and targets are never used by trainer. Previously inspected results informed this iteration; this is development evaluation.",
        "negativeGroups": "Unsupported generator categories all stay in training; authored evaluation supplies separate unsupported/ambiguous requests. Generated development contains supported meanings only.",
        "policy": "The named holdout files are now development benchmarks, evaluated outside the trainer. Do not claim untouched generalization from them. A fresh collection is needed for a fresh generalization claim.",
    }
