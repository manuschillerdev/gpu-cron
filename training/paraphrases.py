...
def main():
    ...
    if args.command == "prompts":
        ...
        for parent in parents.values():
            if parent["target"]["status"] != "supported" or parent["groupId"] in groups:
                continue
            groups.add(parent["groupId"])
    ...
    proposals = [
        json.loads(line) for line in args.file.read_text().splitlines() if line.strip()
    ]
    ...
    frozen = (
        {r["id"]: r for r in map(json.loads, parent_path.read_text().splitlines())}
    ...
    for proposal in proposals:
        ...
        if "target" in proposal and proposal["target"] != parent["target"]:
            raise ValueError("Teacher must not replace the parent meaning")
        parts = [(p["text"], p["role"]) for p in proposal["parts"]]
        if any(
            role not in ["prefix", "recurrence", "time", "exclusion", "unknown"]
            for _, role in parts
        ):
    ...
    print(
        json.dumps(
            {
                "imported": len(proposals),
                "total": len(by_id),
                "note": "Parent meaning/group inherited. Schema checks do not prove semantic fidelity; provenance remains visible. No review gate.",
            }
        )
    )
